...
export class BddScenarioExtractorTool {
  ...
  private buildPrompt(
    ...
    const fileContents = repoFiles
      .map(f => {
        // --- START DEBUG LOGGING ---
        console.log(`[BddExtractor DEBUG] Processing file map item. Type of f: ${typeof f}, f.description type: ${typeof f.description}`);
        if (typeof f.description === 'function') {
            console.error('[BddExtractor FATAL] f.description is a function! Object:', f);
        }
        // --- END DEBUG LOGGING ---
...
5.  **Finding the \`request\` path**: For the \`request\` field in each scenario, you MUST construct the relative path to its XML message file. The path typically follows this pattern: \`messages/{operationName}/{scenario_status}/request.xml\`. Infer the \`{scenario_status}\` (e.g., 'Success', 'SystemException') from the scenario's name or description. For example, a scenario named 'errorDeSistemaEnrolarMedioDePago' implies the status is 'SystemException'.
