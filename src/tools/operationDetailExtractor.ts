...
## INSTRUCTIONS
1.  **CRITICAL: Generate Description**: You MUST provide a value for the \`description\` field. If you cannot find a comment or header that describes the operation, you MUST generate a concise, one-sentence description based on the operation name. For example, for the operation 'enrolarMedioDePago', generate a description like 'Operation to enroll a payment method.' DO NOT return \`null\` for the description.

2.  **CRITICAL: Find Validation Paths**: You MUST find the relative paths for the WSDL and feature files.
    - Populate \`validationSources.wsdlFile\` with the path to the .wsdl file. DO NOT leave this field as \`null\`.
    - Populate \`validationSources.featureFile\` with the path to the .feature file. DO NOT leave this field as \`null\`.
...
5.  **Protocol/Action**: If you find a ".wsdl" file related to the operation, the 
    \`protocol\` is "SOAP" and the \`action\` is "SOAP". Otherwise, infer "REST" and the appropriate HTTP verb.

6.  **Paths**: For \`requestDataPath\` and \`responseDataPath\`, construct the full relative path from the repository root, for example: \`src/test/resources/messages/${operationName}/Success/request.xml\`.
